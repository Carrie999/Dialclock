...
//                  let textOffsetX = (x2 + x1) / 2
//                  let textOffsetY = (y2 + y1) / 2



                Text("\(romanNumerals[index])")
                    .scaleEffect(x: 0.5, y: 0.9)
//                    .scaleEffect(0.8)
...
struct TenSwiftUIView: View {
    ...
    var body: some View {
        ZStack{
            VStack {
                AnalogClockView10(foregroundColor: $themeColor,foreSecondColor: $foreSecondColor,foreThirdColor:$foreThirdColor) { date in
                    currentTime = date
