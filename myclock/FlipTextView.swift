...
struct FlipTextView: View {
  ...
  var body: some View {
    VStack(spacing: 0) {
      ZStack {
        FlipTextViewHalf(text: currentValue, type: .top, config: config).offset(y:-0)
...
//      config.textColor.frame(height: config.separatorHeight)
      ZStack {
        FlipTextViewHalf(text: previousValue, type: .bottom, config: config).offset(y:0)
...
//    .overlay(
//        content:{
//            Rectangle().frame(height:4).offset(y:2)
//        }
//    )
    .fixedSize()
    .onChange(of: value) { 
...
struct FlipTextView_Previews: PreviewProvider {
  static var previews: some View {
    FlipTextView(.constant("2")).preferredColorScheme(.light)
